...
import styles from './SaveProductStyles';
...
export default class SaveProduct extends React.Component<Props>{
  ...
  addIngredients() {
    ...
    if (Product.name && Product.price && Product.image) {
      this.setParam()
      this.props.navigation.navigate('IngredientScreen', { ingredients: this.state.Product.ingredients, page: 'save' })
    } else {
      Alert.alert("Por favor preenche todos os campos...");
    }
  }
  ...
  save() {
    ...
    productService.save_updatePruduct(this.state.token, this.state.Product).then(res => {
      if (res.status == 200) {
        Alert.alert('Salvar com Sucesso')
      } else {
          ...
          <View style={styles.BlocklButtonView}>
            ...
            <View style={styles.SaveButtonView}>
              <TouchableOpacity onPress={() => this.saveProduct()}>
                <Text style={styles.ToucheText}>Save</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
